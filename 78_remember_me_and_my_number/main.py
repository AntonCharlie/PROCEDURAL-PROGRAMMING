...
def get_new_favorite_number(random_number):
	...
	if ans.lower() == "y":
		...
		#handling error kalau bukan integer number yg diinput
	else:
...
def ask_user(data, user):
	if user in data:
		if data[user]:
			...
			# tawaran mau ubah favorit number
			change_number = input("Do you want to change your favorite number? (y/n)")
			if change_number.lower() == "y":
				
			elif change_number.lower() == "n":

			else:
				print("please answer with y or n.")
				return ask_user(data, user)
		else:
